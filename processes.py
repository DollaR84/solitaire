...
def __thread_png2tex(png, card_x, card_y):
    """Convert png file object in pygame surface with need sizes."""
    image = pygame.image.load(png[1])
    png[1].close()
    return (png[0], pygame.transform.scale(image, (card_x, card_y)))

def svg2png(svg_cards, svg_start_pos, svg_card_size, defs_dict, card_x, card_y):
    """Run process for all svg cards."""
    ...
    with multiprocessing.Pool() as pool:
        results = pool.starmap(__thread_svg2png, zip(svg_cards_string, repeat(svg_start_pos), repeat(svg_card_size), repeat(defs_string)))
        with multiprocessing.pool.ThreadPool() as pool:
            results = pool.starmap(__thread_png2tex, zip(results, repeat(card_x), repeat(card_y)))
